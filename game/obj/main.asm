...
00203$:
	...
	pop	ix
	ret
;src/main.c:579: void updateEnemy(TEnemy* actual) { // maquina de estados
;	---------------------------------
...
_updateEnemy::
	...
	add	hl,sp
	ld	sp,hl
;src/main.c:586: if (actual->engage) { // prioridad a la persecucion, nunca te deja
	ld	c,4 (ix)
	ld	b,5 (ix)
	ld	hl,#0x0015
	add	hl,bc
	...
	or	a, a
	jr	Z,00115$
;src/main.c:587: enemy->patrolling = 0;
	ld	hl,#(_enemy + 0x000a)
	ld	(hl),#0x00
;src/main.c:588: engage(actual, prota.x, prota.y);
	ld	a, (#_prota + 1)
	ld	hl, #_prota + 0
	ld	d,(hl)
	push	af
	...
	jp	00117$
00115$:
;src/main.c:590: lookFor(actual); // actualiza si el enemigo tiene el prota al alcance o lo ha visto
	push	bc
	push	bc
	call	_lookFor
	pop	af
	pop	bc
;src/main.c:596: actual->patrolling = 0;
	ld	hl,#0x000A
	add	hl,bc
	ld	-2 (ix),l
	ld	-1 (ix),h
;src/main.c:591: if (actual->patrolling) {
	ld	l,-2 (ix)
	ld	h,-1 (ix)
	ld	l,(hl)
;src/main.c:594: if (actual->in_range) {
	ld	a,c
	add	a, #0x10
	ld	e,a
	ld	a,b
...
;src/main.c:591: if (actual->patrolling) {
	...
	or	a, a
	jp	Z,00112$
;src/main.c:593: moverEnemigoPatrol(actual);
	push	bc
	push	de
	push	bc
...
;src/main.c:596: actual->patrolling = 0;
	...
	ld	h,-1 (ix)
	ld	(hl),#0x00
;src/main.c:597: actual->engage = 1;
	pop	hl
	push	hl
	ld	(hl),#0x01
	jp	00117$
00104$:
;src/main.c:598: } else if (actual->seen) {
	ld	hl,#0x0011
	add	hl,bc
	ld	-7 (ix),l
	ld	-6 (ix),h
	...
	or	a, a
	jp	Z,00117$
;src/main.c:599: pathFinding(actual->x, actual->y, prota.x , prota.y, actual, mapa);
	ld	a,(#_prota + 1)
	ld	-8 (ix),a
	ld	hl, #_prota + 0
	ld	e,(hl)
	...
	ld	sp,hl
	pop	bc
;src/main.c:600: actual->p_seek_x = actual->x;
	ld	hl,#0x0017
	add	hl,bc
	ex	de,hl
	ld	a,(bc)
	ld	(de),a
;src/main.c:601: actual->p_seek_y = actual->y;
	ld	hl,#0x0018
	add	hl,bc
	ex	de,hl
	ld	l,-10 (ix)
...
;src/main.c:603: actual->iter=0;
	...
	inc	hl
	ld	(hl), a
;src/main.c:604: actual->reversePatrol = NO;
	ld	hl,#0x000B
	add	hl,bc
	ld	(hl),#0x00
;src/main.c:605: actual->patrolling = 0;
	ld	l,-2 (ix)
	ld	h,-1 (ix)
	ld	(hl),#0x00
;src/main.c:606: actual->seen = 0;
	ld	l,-7 (ix)
	ld	h,-6 (ix)
	ld	(hl),#0x00
	jr	00117$
00112$:
;src/main.c:608: } else if (actual->seek) {
	ld	l,-4 (ix)
	ld	h,-3 (ix)
	ld	a,(hl)
	or	a, a
	jr	Z,00117$
;src/main.c:609: moverEnemigoSeek(actual);
	push	bc
	push	de
	push	bc
	...
	pop	de
	pop	bc
;src/main.c:610: if (actual->in_range) {
	ld	a,(de)
	or	a, a
	jr	Z,00117$
;src/main.c:611: engage(actual, prota.x, prota.y);
	ld	a, (#_prota + 1)
	ld	hl, #_prota + 0
	ld	d,(hl)
	push	af
	...
	pop	af
	pop	af
;src/main.c:612: actual->seek = 0;
	ld	l,-4 (ix)
	ld	h,-3 (ix)
	ld	(hl),#0x00
;src/main.c:613: actual->engage = 1;
	pop	hl
	push	hl
	ld	(hl),#0x01
;src/main.c:614: } else if (actual->seen) {
00117$:
	ld	sp, ix
	pop	ix
	ret
;src/main.c:621: void inicializarEnemy() {
;	---------------------------------
...
_inicializarEnemy::
	...
	add	hl,sp
	ld	sp,hl
;src/main.c:622: u8 i = 2 + num_mapa; //sacar distinto numero dependiendo del mapa
	ld	a,(#_num_mapa + 0)
	add	a, #0x02
	ld	-1 (ix),a
...
;src/main.c:634: --i;
	dec	-1 (ix)
;src/main.c:635: actual->x = actual->px = spawnX[i];
	ld	c, e
	ld	b, d
	inc	bc
	inc	bc
	...
	ld	(bc),a
	ld	(de),a
;src/main.c:636: actual->y = actual->py = spawnY[i];
	push	de
	pop	iy
	inc	iy
	ld	c, e
	...
	ld	(bc),a
	ld	0 (iy), a
;src/main.c:637: actual->mover  = NO;
	ld	hl,#0x0006
	add	hl,de
	ld	(hl),#0x00
;src/main.c:638: actual->mira   = M_abajo;
	ld	hl,#0x0007
	add	hl,de
	ld	(hl),#0x03
;src/main.c:639: actual->sprite = g_enemy;
	ld	hl,#0x0004
	add	hl,de
	ld	(hl),#<(_g_enemy)
	inc	hl
	ld	(hl),#>(_g_enemy)
;src/main.c:640: actual->muerto = NO;
	ld	hl,#0x0008
	add	hl,de
	ld	(hl),#0x00
;src/main.c:641: actual->muertes = 0;
	ld	hl,#0x0009
	add	hl,de
	ld	(hl),#0x00
;src/main.c:642: actual->patrolling = SI;
	ld	hl,#0x000A
	add	hl,de
	ld	(hl),#0x01
;src/main.c:643: actual->reversePatrol = NO;
	...
	add	hl,de
	ld	(hl),#0x00
;src/main.c:644: actual->didDamage = 0;
	ld	hl,#0x0016
	add	hl,de
	ld	(hl),#0x00
;src/main.c:645: actual->iter = 0;
	ld	hl,#0x000D
	add	hl,de
	xor	a, a
	ld	(hl), a
	inc	hl
	ld	(hl), a
;src/main.c:646: actual->lastIter = 0;
	ld	hl,#0x000F
	add	hl,de
	ld	(hl),#0x00
;src/main.c:647: actual->seen = 0;
	ld	hl,#0x0011
	add	hl,de
	ld	(hl),#0x00
;src/main.c:648: actual->found = 0;
	ld	hl,#0x0012
	add	hl,de
	ld	(hl),#0x00
;src/main.c:649: actual->engage = 0;
	ld	hl,#0x0015
	add	hl,de
	ld	(hl),#0x00
;src/main.c:650: pathFinding( spawnX[i],  spawnY[i], patrolX[num_mapa + 1][i], patrolY[num_mapa + 1][i], actual, mapa); // calculo rutas de patrulla
	ld	hl,#_num_mapa + 0
	ld	c, (hl)
	inc	c
	ld	b,#0x00
	...
	ld	sp,hl
	pop	de
;src/main.c:651: dibujarEnemigo(actual);
	push	de
	push	de
	call	_dibujarEnemigo
	pop	af
	pop	de
;src/main.c:652: ++actual;
	ld	hl,#0x00E2
	add	hl,de
	ex	de,hl
	jp	00101$
00104$:
	...
	pop	ix
	ret
;src/main.c:656: void avanzarMapa() {
;	---------------------------------
; Function avanzarMapa
; ---------------------------------
_avanzarMapa::
;src/main.c:657: if(num_mapa < NUM_MAPAS -1) {
	ld	a,(#_num_mapa + 0)
	sub	a, #0x02
	jr	NC,00102$
;src/main.c:658: mapa = mapas[++num_mapa];
	ld	bc,#_mapas+0
	ld	hl, #_num_mapa+0
	inc	(hl)
	ld	iy,#_num_mapa
	...
	ld	a,(hl)
	ld	(#_mapa + 1),a
;src/main.c:659: prota.x = prota.px = 2;
	ld	hl,#(_prota + 0x0002)
	ld	(hl),#0x02
	ld	hl,#_prota
	ld	(hl),#0x02
;src/main.c:660: prota.mover = SI;
	ld	hl,#(_prota + 0x0006)
	ld	(hl),#0x01
;src/main.c:661: dibujarMapa();
	call	_dibujarMapa
;src/main.c:662: inicializarEnemy();
	jp  _inicializarEnemy
00102$:
;src/main.c:665: menuFin(puntuacion);
	ld	iy,#_puntuacion
	ld	l,0 (iy)
	ld	h,#0x00
	jp  _menuFin
;src/main.c:669: void moverIzquierda() {
;	---------------------------------
; Function moverIzquierda
; ---------------------------------
_moverIzquierda::
;src/main.c:670: prota.mira = M_izquierda;
	ld	bc,#_prota+0
	ld	hl,#(_prota + 0x0007)
	ld	(hl),#0x01
;src/main.c:671: if (!checkCollision(M_izquierda)) {
	push	bc
	ld	a,#0x01
	push	af
	inc	sp
	...
	or	a, a
	ret	NZ
;src/main.c:672: prota.x--;
	ld	a,(bc)
	add	a,#0xFF
	ld	(bc),a
;src/main.c:673: prota.mover = SI;
	ld	hl,#(_prota + 0x0006)
	ld	(hl),#0x01
;src/main.c:674: prota.sprite = g_hero_left;
	ld	hl,#_g_hero_left
	ld	((_prota + 0x0004)), hl
	ret
;src/main.c:678: void moverDerecha() {
;	---------------------------------
; Function moverDerecha
; ---------------------------------
_moverDerecha::
;src/main.c:679: prota.mira = M_derecha;
	ld	hl,#(_prota + 0x0007)
	ld	(hl),#0x00
;src/main.c:680: if (!checkCollision(M_derecha) && ( prota.x + G_HERO_W < 80)) {
	xor	a, a
	push	af
	inc	sp
	call	_checkCollision
	...
	or	a,e
	jr	Z,00104$
;src/main.c:681: prota.x++;
	inc	c
	ld	hl,#_prota
	ld	(hl),c
;src/main.c:682: prota.mover = SI;
	ld	hl,#(_prota + 0x0006)
	ld	(hl),#0x01
;src/main.c:683: prota.sprite = g_hero;
	ld	hl,#_g_hero
	ld	((_prota + 0x0004)), hl
	ret
00104$:
;src/main.c:684: }else if( prota.x + G_HERO_W >= 80){
	ld	a,e
	or	a, a
	ret	NZ
;src/main.c:685: avanzarMapa();
	jp  _avanzarMapa
;src/main.c:689: void moverArriba() {
;	---------------------------------
; Function moverArriba
; ---------------------------------
_moverArriba::
;src/main.c:690: prota.mira = M_arriba;
	ld	hl,#(_prota + 0x0007)
	ld	(hl),#0x02
;src/main.c:691: if (!checkCollision(M_arriba)) {
	ld	a,#0x02
	push	af
	inc	sp
	call	_checkCollision
...
;src/main.c:693: prota.y--;
	dec	c
	ld	(hl),c
;src/main.c:694: prota.mover  = SI;
	ld	hl,#(_prota + 0x0006)
	ld	(hl),#0x01
;src/main.c:695: prota.sprite = g_hero_up;
	ld	hl,#_g_hero_up
	ld	((_prota + 0x0004)), hl
	ret
;src/main.c:699: void moverAbajo() {
;	---------------------------------
; Function moverAbajo
; ---------------------------------
_moverAbajo::
;src/main.c:700: prota.mira = M_abajo;
	ld	hl,#(_prota + 0x0007)
	ld	(hl),#0x03
;src/main.c:701: if (!checkCollision(M_abajo) ) {
	ld	a,#0x03
	push	af
	inc	sp
	call	_checkCollision
...
;src/main.c:703: prota.y++;
	inc	a
	ld	(bc),a
;src/main.c:704: prota.mover  = SI;
	ld	hl,#(_prota + 0x0006)
	ld	(hl),#0x01
;src/main.c:705: prota.sprite = g_hero_down;
	ld	hl,#_g_hero_down
	ld	((_prota + 0x0004)), hl
	ret
;src/main.c:712: void intHandler() {
;	---------------------------------
; Function intHandler
; ---------------------------------
_intHandler::
;src/main.c:713: if (++i == 6) {
	ld	hl, #_i+0
	inc	(hl)
	ld	a,(#_i + 0)
	sub	a, #0x06
...
;src/main.c:715: i=0;
	...
	ld	(hl), #0x00
	ret
;src/main.c:719: void inicializarCPC() {
;	---------------------------------
; Function inicializarCPC
; ---------------------------------
_inicializarCPC::
;src/main.c:720: cpct_disableFirmware();
	call	_cpct_disableFirmware
;src/main.c:721: cpct_setVideoMode(0);
	ld	l,#0x00
	call	_cpct_setVideoMode
;src/main.c:722: cpct_setBorder(HW_BLACK);
	ld	hl,#0x1410
	push	hl
	call	_cpct_setPALColour
;src/main.c:723: cpct_setPalette(g_palette, 16);
	ld	hl,#0x0010
	push	hl
	ld	hl,#_g_palette
	push	hl
...
; ---------------------------------
_inicializarJuego::
;src/main.c:730: num_mapa = 0;
	ld	hl,#_num_mapa + 0
	ld	(hl), #0x00
;src/main.c:731: mapa = mapas[num_mapa];
	ld	hl, #_mapas + 0
	ld	a,(hl)
	ld	iy,#_mapa
	ld	0 (iy),a
	...
	ld	a,(hl)
	ld	(#_mapa + 1),a
;src/main.c:732: cpct_etm_setTileset2x4(g_tileset);
	ld	hl,#_g_tileset
	call	_cpct_etm_setTileset2x4
;src/main.c:734: dibujarMapa();
	call	_dibujarMapa
;src/main.c:737: barraPuntuacionInicial();
	call	_barraPuntuacionInicial
;src/main.c:740: prota.x = prota.px = 4;
	ld	hl,#(_prota + 0x0002)
	ld	(hl),#0x04
	ld	hl,#_prota
	ld	(hl),#0x04
;src/main.c:741: prota.y = prota.py = 80 + ORIGEN_MAPA_Y;
	ld	hl,#(_prota + 0x0003)
	ld	(hl),#0x68
	ld	hl,#(_prota + 0x0001)
	ld	(hl),#0x68
;src/main.c:742: prota.mover  = NO;
	ld	hl,#(_prota + 0x0006)
	ld	(hl),#0x00
;src/main.c:743: prota.mira=M_derecha;
	ld	hl,#(_prota + 0x0007)
	ld	(hl),#0x00
;src/main.c:744: prota.sprite = g_hero;
	ld	hl,#_g_hero
	ld	((_prota + 0x0004)), hl
;src/main.c:748: cu.x = cu.px = 0;
	ld	hl,#(_cu + 0x0002)
	ld	(hl),#0x00
	ld	hl,#_cu
	ld	(hl),#0x00
;src/main.c:749: cu.y = cu.py = 0;
	ld	hl,#(_cu + 0x0003)
	ld	(hl),#0x00
	ld	hl,#(_cu + 0x0001)
	ld	(hl),#0x00
;src/main.c:750: cu.lanzado = NO;
	ld	hl,#(_cu + 0x0006)
	ld	(hl),#0x00
;src/main.c:751: cu.mover = NO;
	ld	hl,#(_cu + 0x0009)
	ld	(hl),#0x00
;src/main.c:752: cu.off_bound = NO;
	ld	hl,#(_cu + 0x000a)
	ld	(hl),#0x00
;src/main.c:754: inicializarEnemy();
	call	_inicializarEnemy
;src/main.c:756: dibujarProta();
	jp  _dibujarProta
;src/main.c:759: void main(void) {
;	---------------------------------
; Function main
; ---------------------------------
_main::
...
;src/main.c:787: if(!actual->muerto){
	...
	or	a, a
	jr	NZ,00107$
;src/main.c:788: if (!actual->didDamage)
	ld	a,-4 (ix)
	add	a, #0x16
	ld	c,a
	ld	a,-3 (ix)
	...
	or	a, a
	jr	NZ,00104$
;src/main.c:789: updateEnemy(actual);
	ld	l,-4 (ix)
	ld	h,-3 (ix)
	push	hl
	call	_updateEnemy
	...
	jr	00107$
00104$:
;src/main.c:791: actual->didDamage = 0;
	xor	a, a
	ld	(bc),a
00107$:
;src/main.c:793: ++actual;
	ld	a,-4 (ix)
	add	a, #0xE2
	ld	-4 (ix),a
	ld	a,-3 (ix)
...
;src/main.c:799: if (prota.mover) {
	ld	bc,#_prota + 6
	ld	a,(bc)
	or	a, a
	jr	Z,00112$
;src/main.c:800: redibujarProta();
	push	bc
...
;src/main.c:801: prota.mover = NO;
	...
	ld	(bc),a
00112$:
;src/main.c:803: if(cu.lanzado && cu.mover){
	ld	hl, #(_cu + 0x0006) + 0
	ld	c,(hl)
;src/main.c:804: redibujarCuchillo(cu.eje, cu.x, cu.y, &cu, g_tablatrans, mapa);
;src/main.c:803: if(cu.lanzado && cu.mover){
	ld	a,c
	or	a, a
	jr	Z,00117$
...
;src/main.c:804: redibujarCuchillo(cu.eje, cu.x, cu.y, &cu, g_tablatrans, mapa);
	...
	jr	00118$
00117$:
;src/main.c:805: }else if (cu.lanzado && !cu.mover){
	ld	a,c
	or	a, a
	jr	Z,00118$
	ld	a, (#(_cu + 0x0009) + 0)
	or	a, a
	jr	NZ,00118$
;src/main.c:806: borrarCuchillo(cu.eje, cu.x, cu.y, mapa);
	ld	hl, #(_cu + 0x0001) + 0
	ld	b,(hl)
	ld	hl, #_cu + 0
	ld	c,(hl)
	...
	pop	af
	inc	sp
;src/main.c:807: cu.lanzado = NO;
	ld	hl,#(_cu + 0x0006)
	ld	(hl),#0x00
00118$:
;src/main.c:810: i = 2 + num_mapa;
	ld	a,(#_num_mapa + 0)
	add	a, #0x02
	ld	-2 (ix),a
;src/main.c:811: actual = enemy;
	ld	bc,#_enemy
;src/main.c:812: while(i){
00125$:
	ld	a,-2 (ix)
	or	a, a
	jp	Z,00127$
;src/main.c:814: --i;
	dec	-2 (ix)
;src/main.c:815: if(actual->mover){
	ld	hl,#0x0006
	add	hl,bc
	ld	-4 (ix),l
	ld	-3 (ix),h
	...
	or	a, a
	jr	Z,00121$
;src/main.c:816: redibujarEnemigo((*actual).px, (*actual).py, actual);
	ld	l, c
	ld	h, b
	inc	hl
	inc	hl
	...
	pop	bc
00121$:
;src/main.c:819: if (actual->muerto && actual->muertes == 0){
	push	bc
	pop	iy
	ld	a,8 (iy)
	or	a, a
	...
	or	a, a
	jr	NZ,00123$
;src/main.c:820: borrarEnemigo((*actual).x, (*actual).y);
	ld	hl,#0x0001
	add	hl,bc
	ex	(sp), hl
	pop	hl
	...
	pop	de
	pop	bc
;src/main.c:821: dibujarExplosion(actual);
	push	bc
	push	de
	push	bc
	...
	pop	de
	pop	bc
;src/main.c:822: puntuacion_aux = puntuacion;
	ld	iy,#_puntuacion
	ld	l,0 (iy)
	ld	h,#0x00
;src/main.c:823: puntuacion = aumentarPuntuacion(puntuacion_aux);
	push	bc
	push	de
	push	hl
	...
	ld	iy,#_puntuacion
	ld	0 (iy),l
;src/main.c:824: modificarPuntuacion(puntuacion);
	ld	iy,#_puntuacion
	ld	l,0 (iy)
	ld	h,#0x00
	push	bc
	...
	pop	de
	pop	bc
;src/main.c:825: actual->mover = NO;
	ld	l,-4 (ix)
	ld	h,-3 (ix)
	ld	(hl),#0x00
;src/main.c:826: actual->muertes++;
	ld	a,(de)
	inc	a
	ld	(de),a
;src/main.c:827: actual->x = 0;
	xor	a, a
	ld	(bc),a
;src/main.c:828: actual->y = 0;
	pop	hl
	push	hl
	ld	(hl),#0x00
00123$:
;src/main.c:830: ++actual;
	ld	hl,#0x00E2
	add	hl,bc
	ld	c,l
	ld	b,h
